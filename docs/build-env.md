...
firmware](../README.md#building) for the cloud-based alternative, which requires no local setup at
all.
...
1. Fork this repository on GitHub, then clone *your fork*. I like to name my local clone
   ...
   ```bash
   # Replace `rcopra` with your username
   git clone https://github.com/rcopra/zmk-config zmk-workspace
   ```
...
firmware for all matching board and shield combinations. For instance, to build the firmware for the
Hillside D50, type `just build hillside`, which builds all `hillside_d50_*` targets. (`just list`
...
`modules/zmk/`), the same environment doubles as a development environment for [ZMK
modules](https://github.com/topics/zmk-module).
...
firmware](https://github.com/urob/zmk-config#pinning-the-firmware).
...
tested](https://github.com/rcopra/zmk-config/actions/workflows/test-build-env.yml) on Linux and
macOS.)
